...
void main() {
  group('PermissionDef models', () {
    test('AndroidPermissionDef.matches handles full and short keys (case-insensitive)', () {
      const def = AndroidPermissionDef('android.permission.CAMERA', runtime: true, group: 'camera');
    ...
    test('AndroidPermissionDef equality and hashCode', () {
      const a = AndroidPermissionDef('android.permission.INTERNET', runtime: false, group: 'network');
      const b = AndroidPermissionDef('android.permission.INTERNET', runtime: false, group: 'network');
      expect(a, equals(b));
      expect(a.hashCode, equals(b.hashCode));

      const c = AndroidPermissionDef('android.permission.INTERNET', runtime: true, group: 'network');
      expect(a == c, isFalse);
    });
    ...
    test('IosPermissionDef equality and hashCode', () {
      const x = IosPermissionDef('NSFoo', group: 'misc', minimumIosVersion: '13.0', successorOf: 'NSBar');
      const y = IosPermissionDef('NSFoo', group: 'misc', minimumIosVersion: '13.0', successorOf: 'NSBar');
      expect(x, equals(y));
